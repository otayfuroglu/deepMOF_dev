# copied from https://github.com/atomistic-machine-learning/schnetpack/tree/master/src/schnetpack/md/utils
# and edited
...
class Units:
    ...
    # Conversion units use when reading in  inputs
    ...
    def parse_mdunit(unit):
        ...
        if type(unit) == str:
            ...
            scaling = 1.0
            for part in parts:
                if part not in Units.conversions:
                    raise KeyError("Unrecognized unit {:s}".format(part))
                scaling *= Units.conversions[part]
            return scaling
        else:
            return unit
        ...
        if type(unit) == str:
            ...
            for part in parts:
                if part == "/":
                    ...
                    if divide:
                        conversion /= Units.conversions[part]
                        divide = False
                    else:
                        conversion *= Units.conversions[part]
            return conversion
    ...
    def unit2internal(unit):
        conversion = Units._parse_unit(unit)
        return conversion
    ...
    def unit2unit(unit1, unit2):
        conversion1 = Units.unit2internal(unit1)
        conversion2 = Units.internal2unit(unit2)
