...
if mode == "train" or mode == "test":
    ...
    xyzDIR = "/truba_scratch/yzorlu/deepMOF/HDNNP/prepare_data/geomFiles/IRMOFSeries/IRMOF7_linker_torsion36x36"
...
def getOptEneryForces(idx):
    ...
    list_dihedrals_idxs = [[640, 641, 642, 643],
                     [151, 521, 522, 523]] #- 1  correction for ase index, which strats from 0.

    #  dihedrals indexs for IRMOF1
    #  list_dihedrals_idxs = [[92, 532, 533, 536],
    #                                  [495, 514, 515, 518]]

    dihedrals = [[mol.get_dihedral(*list_dihedrals_idx), list_dihedrals_idx] for list_dihedrals_idx in list_dihedrals_idxs]
    c = FixInternals(dihedrals_deg=dihedrals)
    ...
    opt = LBFGS(mol)
    opt.run(fmax=0.09)
    write("{}/Optimized_{:04d}.xyz".format(RESULT_DIR, idx), mol)
...
def getOptEneryForcesFromFiles(idx):
...
def main(n_procs):
    if mode == "train" or mode == "test":
        ...
        #  result_list_tqdm = []
        run_multiprocessing(func=getOptEneryForces,
                                           argument_list=idxs,
        ...
        print("Nuber of %s data points: %d" %(mode, len(idxs)))
        #  result_list_tqdm = []
        run_multiprocessing(func=getOptEneryForcesFromFiles,
                                           argument_list=idxs,
                                           num_processes=n_procs)
    else:
        n_data = len(data)
