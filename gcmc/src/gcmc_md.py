...
class AI_GCMCMD():
    def __init__(self, model_gcmc, model_md, results_dir, interval, atoms_frame, atoms_ads, flex_ads, T, P, fugacity, device, vdw_radii):
        self.model_gcmc = model_gcmc
    ...
    def _set_rigid_triatomic_ads_atoms(self, atoms):
    ...
    def _get_e_ads(self, atoms):
        ...
        for ads_atoms in self._get_ads_atoms(atoms):
            ads_atoms.calc = self.model_gcmc
    ...
    def _init_md(self, atoms, timestep, md_type="nptberendsen"):

        if not self.flex_ads:
            #  self._set_rigid_ads_atoms(atoms)
            self._set_rigid_triatomic_ads_atoms(atoms)

        atoms.calc = self.model_md
        ...
        if md_type.lower() == "nvt":
            ...
            self.dyn = Inhomogeneous_NPTBerendsen(atoms,
                               timestep=timestep*fs,  # Timestep of 1 femtosecond
                               temperature_K=self.T,
                               pressure=self.P,
                               #  taut=0.5e3 *fs,
    ...
    def run(self, timestep, totalsteps, N, X):
        ...
        # initialize MD
        md_steps = 0
        self._init_md(self.atoms, timestep, md_type="nptberendsen")
        #  self._setTqdm(md_steps, totalsteps)
        self._setTqdm(0, totalsteps)
        self.dyn.attach(self._tqdmMD)

        self._set_gcmc(X)
        self.dyn.attach(self._run_gcmc, interval=N)

        #  for N in [N] * int(totalsteps/N):
            #  print("Start MD for %d steps" %N)
        self.dyn.run(totalsteps)

    def _set_gcmc(self, X):

        # setting for gcmc
        if not self.flex_ads:
            #  self.atoms.calc = self.model_gcmc

            atoms_ads = self.atoms_ads.copy()
            atoms_ads_e = self.atoms_ads.copy()
            atoms_ads_e.calc = self.model_gcmc
    ...
    def _run_gcmc(self):
        ...
        self.atoms.calc = self.model_gcmc
        if self.flex_ads:
            e = self.atoms.get_potential_energy() * electronvolt - self._get_e_ads(self.atoms) * electronvolt
        else:
            e = self.atoms.get_potential_energy() * electronvolt - self.e_ads * self.Z_ads # eV to Hartree
        ...
        Z_ads_in_loop = 0
        print("Start GCMC for %d steps" %self.X)
        for interation in tqdm.trange(self.X):
            ...
            if switch < 0.25:
                ...
                if vdw_overlap(atoms_trial, self.vdw, self.n_frame, self.n_ads, self.Z_ads-1):
                    ...
                    atoms_trial.calc = self.model_gcmc
                    if self.flex_ads:
            ...
            if self.mc_steps % self.interval == 0 and self.old_mc_steps != self.mc_steps:
                write(f'{self.results_dir}/trajectory_{self.P/bar}bar.extxyz', self.atoms, append=True)
                print(f"{self.mc_steps},{self.n_trial_insertion},{self.n_succ_insertion},{self.n_trial_deletion},{self.n_succ_deletion}", file=self.fl_status)
                ...
                self.fl_status.flush()
                self.old_mc_steps = self.mc_steps

        if Z_ads_in_loop > 0:
            velocities = self.atoms.get_velocities()
            add_ads_atoms = self.atoms[-self.n_ads*Z_ads_in_loop:].copy()
            MaxwellBoltzmannDistribution(add_ads_atoms, temperature_K=self.T)
            velocities[-self.n_ads*Z_ads_in_loop:] = add_ads_atoms.get_velocities()
            self.atoms.set_velocities(velocities)

        if not self.flex_ads:
            #  self._set_rigid_ads_atoms(atoms)
            self._set_rigid_triatomic_ads_atoms(self.atoms)
